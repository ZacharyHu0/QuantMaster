...
统一滚动协议默认使用 756 个交易日训练、244 日测试、每 244 日向前滚动一次，训练与
测试之间保留不短于最长预测周期的 purge；三个开发期 OOS 窗口用于选择，最后一个窗口
保持密封。训练、测试、步长、purge 和开发窗口数可在设置中心调整。历史不足时任务明确
报告“证据不足”、当前/所需交易日与可调整字段，不会自动缩短周期或回看未来答案。
...
变化榜和共振；“行业周期”是唯一展示趋势向上占比 × 低位偏弱占比坐标的位置；“细分题材”
提供全目录搜索、阶段筛选及每页 50 条明细。行业与题材评分随窗口变化，其中趋势状态、上涨
宽度和量能占 75%，同层级相对收益与轮动变化占 25%；“ETF 研究”保留原资金流兼容视图，
并覆盖全部沪深 ETF，按宽基、行业主题、策略、QDII、债券、商品和货币分别比较趋势、
相对强弱、流动性、回撤及份额证据。分钟指标只是非评分补充，份额日期和滞后会逐只披露。
生命周期阶段始终按连续 3 日判断，不随观察窗口改变。
