...
输入框会先用本地证券主数据确认名称和市场；提交后页面按标的解析、行情、技术指标、基本面、
资讯/资金、心理/宏观和综合判断显示真实进度，完成后呈现六维证据、情景触发和总风险清单。

已经绑定的飞书私聊或群聊也可以发送“分析贵州茅台”“六维分析 600519”或
“贵州茅台怎么样”。Bot 只发一张进度卡，并把它原位更新为完整报告；群聊仍需真实
`@QuantMaster`。上游 skill 的持仓监控脚本不会执行，分析只读取本次明确询问的单一标的。
本地没有财务、资讯或逐单资金流时，对应维度会标为部分数据或数据缺失，不代表现实中没有事件，
也不会用模型虚构数字。若已配置 LLM，模型只归纳结构化证据；不可用时自动保留规则版完整报告。
