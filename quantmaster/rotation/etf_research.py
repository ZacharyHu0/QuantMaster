...
def classify_etf(
    name: str, *, benchmark: str = "", fund_type: str = "", invest_type: str = "",
) -> tuple[str, tuple[str, ...]]:
    ...
    rules = (
        ...
        ("港股及海外 QDII", (
            "QDII", "港股", "恒生", "H股", "中概", "纳指", "标普", "日经",
            "德国", "法国", "沙特", "印度", "东南亚", "海外",
        )),
        ("策略", ("红利", "低波", "价值", "成长", "质量", "策略", "增强", "SMART")),
        ("境内宽基", (
            "沪深300", "中证500", "中证1000", "中证2000", "A500", "上证50",
            "科创50", "创业板", "深证100", "中证A", "全指", "综指",
        )),
    )
...
class EtfResearchStore:
    ...
    def publish(self, snapshot: EtfResearchSnapshot) -> EtfResearchSnapshot:
        ...
        with self._lock:
            ...
            if target.exists():
                existing = EtfResearchSnapshot.from_dict(json.loads(
            ...
            _atomic_text(self.root / "latest.json", json.dumps({
                "snapshot_id": snapshot.snapshot_id, "last_failure": {},
            }, ensure_ascii=False, indent=2))
            return snapshot

    def get(self, snapshot_id: str) -> EtfResearchSnapshot | None:
        try:
            value = json.loads((self.root / "snapshots" / f"{snapshot_id}.json").read_text(
                encoding="utf-8"
            ))
    ...
    def latest(self) -> EtfResearchSnapshot | None:
        ...
        if snapshot is not None and failure:
            ...
            data["staleness"] = {
                "stale": True, "reason": str(failure.get("reason") or "ETF 研究刷新失败"),
    ...
    def history(self, limit: int = 50) -> list[dict[str, Any]]:
        paths = sorted((self.root / "snapshots").glob("*.json"),
        ...
        for path in paths[:max(1, limit)]:
            try:
                ...
                result.append({
                    "snapshot_id": value["snapshot_id"], "ingest_id": value.get("ingest_id", ""),
                    "as_of_date": value["as_of_date"], "generated_at": value["generated_at"],
                    "coverage": value.get("coverage") or {}, "item_count": len(value.get("items") or []),
    ...
    def record_failure(self, reason: str) -> None:
        ...
        _atomic_text(self.root / "latest.json", json.dumps({
            ...
            "last_failure": {
                "reason": reason[:500], "attempted_at": datetime.now(UTC).isoformat(timespec="seconds"),
            },
        }, ensure_ascii=False, indent=2))
...
class EtfResearchService:
    ...
    def profiles(self) -> list[EtfProfile]:
        ...
        if not observations.empty:
            for symbol, group in observations.groupby("symbol"):
                ...
                metadata[str(symbol).upper()] = {
                    key: str(last.get(key) or "") for key in
                    ("benchmark", "fund_type", "invest_type")
                }
        result = []
        for instrument in self.instruments.list(market="CN"):
            ...
            category, evidence = classify_etf(
                instrument.name, benchmark=extra.get("benchmark", ""),
                fund_type=extra.get("fund_type", ""), invest_type=extra.get("invest_type", ""),
            )
            result.append(EtfProfile(
                symbol=instrument.symbol, name=instrument.name, category=category,
                benchmark=extra.get("benchmark", ""), fund_type=extra.get("fund_type", ""),
                invest_type=extra.get("invest_type", ""), list_date=instrument.list_date,
                status=instrument.status, classification_evidence=evidence,
            ))
    ...
    def _minute_metrics(frame: pd.DataFrame) -> dict[str, dict[str, Any]]:
        ...
        for symbol, group in frame.groupby("symbol"):
            ...
            result[str(symbol)] = {
                "rows": len(values), "complete_session": len(values) >= 240,
                ...
                "intraday_drawdown": float((close / close.cummax() - 1).min()) if close.notna().any() else None,
    ...
    def _daily_metrics(group: pd.DataFrame) -> dict[str, Any]:
        ...
        returns = close.pct_change().replace([np.inf, -np.inf], np.nan)
        metric: dict[str, Any] = {
            "sessions": len(close), "close": float(close.iloc[-1]) if len(close) else None,
            ...
            "stockdb_share_change_5d": (
                ...
                if shares.notna().sum() >= 6 and shares.dropna().iloc[-6] else None
            ),
        }
    ...
    def scan(
        self, *, as_of: str = "", progress: Progress | None = None,
        ...
        cache_key = content_hash({
            ...
            "asset": "etf", "artifact": identity.artifact_id, "master": master_id,
            "start": str(start.date()), "end": str(end.date()), "symbols": symbols,
        })
        ingest = next((item for item in self.ingest_store.history(100)
                       if item.provenance.get("cache_key") == cache_key and "etf" in item.assets), None)
        daily = pd.DataFrame()
        ...
        if ingest is not None:
            ...
            cached_snapshot_id = "etf_" + hashlib.sha256(
        ...
        if daily.empty:
            ...
            for offset in range(0, len(symbols), 300):
                ...
                frames.append(self.source.daily_cross_section(
                    batch, str(start.date()), str(end.date()),
                ))
                progress(5 + int(50 * (offset + len(batch)) / len(symbols)),
                         "读取 ETF 日线", f"{offset + len(batch)}/{len(symbols)}")
            ...
            latest = daily[pd.to_datetime(daily["date"], errors="coerce").dt.date == date.fromisoformat(actual)]
            observed = int(latest["symbol"].nunique())
            ratio = observed / len(symbols)
            required_ratio = float(latest[["open", "high", "low", "close", "volume"]].notna().all(axis=1).mean())
            ...
            if cfg.free_stockdb_etf_minutes_enabled:
                ...
                for offset in range(0, len(symbols), 300):
                    ...
                    minute_frames.append(self.source.intraday_many(
                        symbols[offset:offset + 300], minute_start, minute_end, "1m",
                    ))
                minutes = pd.concat(minute_frames, ignore_index=True) if minute_frames else pd.DataFrame()
            coverage = {
                "status": "complete", "expected_symbols": len(symbols),
                "observed_symbols": observed, "symbol_ratio": round(ratio, 6),
            ...
            ingest = self.ingest_store.publish_etf(
                daily=daily, minutes=minutes, profiles=[item.to_dict() for item in profiles],
                as_of_date=actual, artifact_id=identity.artifact_id,
                master_snapshot_id=master_id, start_date=str(start.date()), end_date=str(end.date()),
                coverage=coverage, provenance={
                    "cache_key": cache_key, "upstream": "tushare",
                    "distribution": "free-stockdb", "artifact": identity.to_dict(),
        ...
        for profile in profiles:
            ...
            direct_symbol = direct[direct["symbol"].astype(str).str.upper().eq(profile.symbol)] if not direct.empty else pd.DataFrame()
            direct_latest = direct_symbol[direct_symbol["trade_date"].dt.date <= date.fromisoformat(actual)].sort_values("trade_date") if not direct_symbol.empty else direct_symbol
            if not direct_latest.empty and pd.Timestamp(direct_latest.iloc[-1]["trade_date"]).date().isoformat() == actual:
                latest_share = pd.to_numeric(pd.Series([direct_latest.iloc[-1].get("shares")]), errors="coerce").iloc[0]
                ...
                if len(direct_latest) >= 2 and metric.get("close") is not None:
                    prior = pd.to_numeric(pd.Series([direct_latest.iloc[-2].get("shares")]), errors="coerce").iloc[0]
                ...
                symbol_dates = sorted(pd.to_datetime(
                    daily.loc[daily["symbol"].eq(profile.symbol), "date"], errors="coerce",
                ...
                metric["flow"] = metric.get("stockdb_flow")
            rows.append({"profile": profile, "metrics": metric,
                         "effective_date": effective_date, "lag": lag, "share_source": share_source})
        ...
        for category in ETF_CATEGORIES:
            ...
            if not eligible.empty:
                def pct(column: str, ascending: bool = True) -> pd.Series:
                    return pd.to_numeric(eligible.get(column), errors="coerce").rank(
                ...
                eligible["score"] = 100 * (
                    .25 * pct("return_20d") + .15 * pct("return_5d")
                    + .20 * pct("avg_amount_20d") + .15 * pct("drawdown_20d")
                    + .10 * pct("volatility_20d", ascending=False)
                    + .15 * pct("stockdb_share_change_5d")
                )
        ...
        input_hash = content_hash({
            "ingest_id": ingest.ingest_id, "profiles": [item.to_dict() for item in profiles],
            "score_version": ETF_SCORE_VERSION,
        })
        snapshot_id = "etf_" + hashlib.sha256(
            f"{actual}:{ETF_SCORE_VERSION}:{input_hash}".encode()
        ).hexdigest()[:24]
        for row in rows:
            profile, metric = row["profile"], dict(row["metrics"])
            ranked = category_frames.get(profile.category)
            rank_row = ranked.loc[profile.symbol] if ranked is not None and profile.symbol in ranked.index else None
            ...
            items.append(EtfResearchItem(
                symbol=profile.symbol, name=profile.name, category=profile.category,
                category_rank=(int(rank_row["category_rank"]) if rankable and pd.notna(rank_row.get("category_rank")) else None),
                score=(round(float(rank_row["score"]), 4) if rankable and pd.notna(rank_row.get("score")) else None),
                rankable=rankable, excluded_reason="" if rankable else "日线不足 21 日或流动性缺失",
                metrics=metric, minute_evidence=minute_metrics.get(profile.symbol, {
                    "rows": 0, "complete_session": False, "scoring_input": False,
                }), shares_effective_date=row["effective_date"], share_lag_sessions=row["lag"],
                coverage={"daily": profile.symbol in by_symbol, "minute": profile.symbol in minute_metrics},
                provenance={
                    "price": "tushare:via-free-stockdb", "shares": row["share_source"],
                    "classification": "tushare-master+quantmaster-rules",
                    "independent_cross_validation": False,
                }, as_of_date=actual, snapshot_id=snapshot_id,
                ingest_id=ingest.ingest_id, artifact_id=ingest.artifact_id,
            ))
        items.sort(key=lambda item: (
            ETF_CATEGORIES.index(item.category), item.category_rank or 10**9, item.symbol,
        ))
        snapshot = EtfResearchSnapshot(
            snapshot_id=snapshot_id, ingest_id=ingest.ingest_id,
            artifact_id=ingest.artifact_id, as_of_date=actual,
            coverage={**ingest.coverage, "rankable_symbols": sum(item.rankable for item in items)},
            provenance={
                "upstream": "tushare", "distribution": "free-stockdb",
                "master": "tushare:instrument-store", "calculation": "QuantMaster",
                "independent_cross_validation": False,
            }, items=tuple(items), categories=tuple(
