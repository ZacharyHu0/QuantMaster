...
# "ac_" + sha256 hex digest.  Constrain the path parameter so
# obviously-wrong literal paths (e.g. /snapshots/after-close)
# do not match this route and fall through to a framework 404.
...
@router.get(f"/snapshots/{{snapshot_id:regex({_SNAPSHOT_ID_PATTERN})}}")
...
@router.get(f"/export/{{snapshot_id:regex({_SNAPSHOT_ID_PATTERN})}}")
def export_snapshot(
