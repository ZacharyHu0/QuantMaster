"""Capability routes owned by the transport layer.

The composition root imports this router; domain implementations stay behind
their existing application seams.
"""
...
def shutdown_stream_runtime(timeout: float = 5.0) -> None:
    global _web_stream_runtime
    with _web_stream_lock:
...
def _stream_emit(
    events: queue.Queue[dict | None], cancelled: threading.Event, request_id: str,
...
def _run_stream_task(
    ...
    try:
        events.put({"type": "result", "data": task(emit), "request_id": request_id})
        ...
        event: dict[str, Any] = {
            ...
            "problem": exc.problem, "error_id": request_id, "request_id": request_id,
        }
        ...
        events.put({
            "type": "error", "message": str(exc), "problem": problem,
        ...
        problem = make_problem(
            "stream_task_failed", source="后台任务", title="数据任务未完成", message=message,
            action="重试一次；如仍失败，请复制请求编号排查后端日志。", blocking=True,
        )
        events.put({
            "type": "error", "message": message, "problem": problem,
            "error_id": request_id, "request_id": request_id,
        })
    finally:
        events.put(None)
...
async def _stream_events(
    events: queue.Queue[dict | None], cancelled: threading.Event,
) -> AsyncIterator[str]:
    try:
        while True:
...
def _progress_stream(
    ...
    def bounded_run() -> None:
        try:
            def emit(*args: Any, **kwargs: Any) -> None:
                _stream_emit(events, cancelled, request_id, *args, **kwargs)

            _run_stream_task(task, emit, events, request_id)
        finally:
    ...
    return StreamingResponse(
        _stream_events(events, cancelled),
...
@router.post("/api/v1/diagnostics/providers/{lane}/probe")
...
@router.get("/api/v1/market/fear-greed")
...
def market_history(
    ...
    try:
        market_envelope = data_api.read_bars(symbol, start, end, frequency=frequency)
...
def market_regime(req: RegimeRequest) -> dict:
    ...
    try:
        ...
        market_envelope = data_api.read_panel(list(universe_snapshot.symbols), req.start, end)
...
@router.post("/api/v1/research/selection/daily")
...
@router.get("/api/v1/research/selection/history")
def selection_history(
    ...
    names = data_api.read_stock_names(symbols) if symbols else {}
    for snapshot in snapshots:
        for pick in snapshot.get("picks", []):
...
@router.post("/api/v1/research/decision/dashboard")
...
def _read_decision_panel(
    data_api: Any, req: DecisionDashboardRequest, symbols: list[str], end: str,
    ...
    def on_symbol(completed: int, total: int, symbol: str, success: bool) -> None:
        if progress:
            progress(
                ...
                {
                    "kind": "decision_symbol", "symbol": symbol, "success": success,
                    "completed": completed, "total": total, "read_mode": "local_only",
                },
    ...
    if req.save and not market_envelope.quality.formal_eligible:
        ...
        try:
            refreshed = data_api.refresh_panel(
                symbols, req.start, end, mode="auto", work_class="normal",
            )
            if refreshed.quality.analysis_eligible and _decision_panel_has_minimum_history(refreshed):
...
def _decision_market_input(
    req: DecisionDashboardRequest, progress: ProgressEmitter | None,
) -> tuple[str, Any, list[str], Any]:
    from quantmaster import data as data_api
    from quantmaster.data.universe import load_universe_analysis_snapshot
    ...
    symbols = list(universe_snapshot.symbols)
    market_envelope = _read_decision_panel(data_api, req, symbols, end, progress)
    return end, universe_snapshot, symbols, market_envelope


def _decision_market_report(
    req: DecisionDashboardRequest, panel: pd.DataFrame, mapping: dict[str, Any],
    progress: ProgressEmitter | None,
) -> dict[str, Any]:
    from quantmaster.market import analyze_market, analyze_sectors
    ...
    if progress:
        ...
        progress(84, "市场状态已就绪", "牛熊、宽度与未来概率可先查看", {
...
def _decision_selection_report(
    req: DecisionDashboardRequest, end: str, universe_snapshot: Any,
    market_envelope: Any, panel: pd.DataFrame, mapping: dict[str, Any],
    names: dict[str, str], industry_evidence: dict[str, Any],
    progress: ProgressEmitter | None,
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, pd.DataFrame], bool]:
    from quantmaster.decision import hybrid_daily_selection, resolve_policy
    ...
    policy = resolve_policy(
        ...
        as_of=end if req.policy_mode == "historical_replay" else "", mode=req.policy_mode,
    )
    if progress:
        progress(92, "决策模型已就绪", policy["profile_label"], {
    ...
    selection = hybrid_daily_selection(
        ...
        policy_snapshot=policy, cap_weight=req.cap_weight, policy_mode=req.policy_mode,
        evidence_sink=decision_feature_inputs,
    )
    selection["calculation_quality"] = selection.get("data_quality")
    market_quality = market_envelope.quality.to_dict()
    market_quality["decision_formal_eligible"] = _decision_market_formal_eligible(market_envelope.quality)
    ...
    persistence = {
        "requested": req.save, "saved": False,
        "status": "not_requested" if not req.save else "pending" if save_allowed else "blocked",
        "reason": "",
    }
    if req.save and not save_allowed:
        persistence["reason"] = "正式决策未保存：行情、候选池或行业证据未通过正式门；计算结果仅供查看"
    selection["persistence"] = persistence
    if progress:
        progress(96, "每日候选已就绪", f"已生成 {len(selection.get('picks', []))} 只候选", {
            "kind": "decision_selection", "selection": selection,
        })
    return selection, market_quality, persistence, decision_feature_inputs, save_allowed


def _decision_history(
    store: Any, req: DecisionDashboardRequest, names: dict[str, str], panel: pd.DataFrame,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    from quantmaster.decision import enrich_decision_snapshots, price_frames_from_panel
    ...
    try:
        ...
        history = enrich_decision_snapshots(
            history, price_frames_from_panel(panel, _decision_history_symbols(history)),
        )
        ...
        issues.append(make_problem(
            ...
            can_continue=True, retryable=True, suggestion="稍后重试历史验证。", component="history",
        ))
...
def _decision_dashboard_data(
    ...
    end, universe_snapshot, symbols, market_envelope = _decision_market_input(req, progress)
    panel = market_envelope.require_data()
    if progress:
    ...
    names = data_api.read_stock_names(symbols)
    market = _decision_market_report(req, panel, mapping, progress)
    selection, market_quality, persistence, decision_feature_inputs, save_allowed = (
        _decision_selection_report(
            req, end, universe_snapshot, market_envelope, panel, mapping, names,
            industry_evidence, progress,
        )
    )
    store = DecisionStore()
    if req.save and save_allowed:
        ...
        persistence.update(saved=True, status="saved")
        selection["persistence"] = persistence
        store.save(selection, req.universe, panel={**panel, **decision_feature_inputs})
    history, issues = _decision_history(store, req, names, panel)
    if progress:
        progress(
            99,
...
@router.post("/api/v1/research/decision/dashboard/stream")
...
@router.get("/api/v1/research/factors")
...
@router.post("/api/v1/research/factors/test")
...
@router.post("/api/v1/research/factors/validate")
...
@router.post("/api/v1/research/mining/genetic")
...
@router.get("/api/v1/portfolio/lists")
def asset_lists_get() -> dict:
    """自选、关注和实盘持有；报价仅复用本地缓存。"""
    return _asset_lists_payload()


@router.post("/api/v1/portfolio/lists/{list_name}")
def asset_lists_add(
...
@router.delete("/api/v1/portfolio/lists/{list_name}/{symbol}")
...
@router.post("/api/v1/portfolio/ledger/trade")
...
@router.post("/api/v1/portfolio/ledger/cashflow")
...
@router.get("/api/v1/portfolio/ledger/report")
...
@router.get("/api/v1/portfolio/ledger/trades")
...
@router.get("/api/v1/portfolio/ledger/nav")
def ledger_get_nav(benchmark: str = "000300.SH") -> dict:
    ...
    for symbol in symbols:
        try:
            envelope = data_api.read_history(symbol, start, end, store=store)
    ...
    try:
        benchmark_envelope = data_api.read_history(benchmark, start, end, store=store)
