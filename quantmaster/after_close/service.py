...
class AfterCloseService:
    ...
    def _universe(self, include_bj: bool) -> list[Instrument]:
        ...
        return [
            item for item in self.instruments.list(market="CN", asset_type="stock")
        ...
        return {
            column: {
                ...
                "latest_ratio": (
                    ...
                    if column in latest and len(latest) else None
                ),
            }
    ...
    def _gate(
        ...
        reasons: list[str] = []
        if frame.empty:
            raise DataGateRejected(["free-stockdb 没有返回日频截面"], {
                "expected_symbols": expected_count, "observed_symbols": 0,
            })
        ...
        required_ratio = (
            0.0 if missing_required
            else float(latest[list(REQUIRED_FIELDS)].notna().all(axis=1).mean())
        )
    ...
    def _frame_hash(frame: pd.DataFrame) -> str:
        columns = [
            column for column in ("symbol", "date", *REQUIRED_FIELDS, *OPTIONAL_FIELDS)
            if column in frame
        ]
    ...
    def _score(
        ...
        *,
    ...
    ) -> tuple[list[SectorRank], list[ResearchCandidate], dict[str, int]]:
        metrics = {
            symbol: self._stock_metrics(group)
        ...
        market20 = _finite(metric_frame["return_20d"].median()) or 0.0
        ...
        board_frame = board_frame.sort_values(
            ["score", "coverage", "code"], ascending=[False, False, True], kind="mergesort",
        ...
        candidate_symbols = [
            symbol for symbol in metric_frame.index
        ...
        ).sort_index(kind="mergesort").sort_values(
            ["score"], ascending=False, kind="mergesort",
        ).head(candidate_limit)
        provenance = {
            "source": "free-stockdb", "calculation": "QuantMaster",
            "score_version": SCORE_VERSION,
        }
        staleness = {"stale": False, "reason": "", "last_attempt_at": ""}
        candidates: list[ResearchCandidate] = []
        for rank, (symbol, row) in enumerate(candidates_frame.iterrows(), 1):
            reasons: list[str] = []
            ...
            candidates.append(ResearchCandidate(
                ...
                metrics={key: _finite(value) for key, value in row.items() if key != "is_st"},
                reasons=tuple(reasons or ["满足全市场流动性与趋势筛选"]),
                exclusion_rules=(
                    "非 ST", f"至少 {min_sessions} 个交易日",
        ...
        for rank, row in board_frame.iterrows():
            sector_results.append(SectorRank(
                ...
                category=row["category"], score=round(float(row["score"]), 4), rank=rank + 1,
                return_5d=_finite(row["return_5d"]), return_20d=_finite(row["return_20d"]),
                relative_20d=_finite(row["relative_20d"]), breadth_20d=_finite(row["breadth_20d"]),
                amount_change=_finite(row["amount_change"]),
                eligible_members=int(row["eligible_members"]), total_members=int(row["total_members"]),
        ...
        return sector_results, candidates, dict(sorted(excluded.items()))
    ...
    def scan(
        ...
        *,
        ...
        try:
            ...
            raise
        if as_of and not all(
            item.get("effective_date") or item.get("as_of_date") for item in boards
        ):
            reasons = [
                "板块目录没有点时生效日期，不能用当前分类强制重算历史选择；"
                "请直接重放已冻结快照"
            ]
        ...
        try:
            sectors, candidates, excluded = self._score(
                frame, boards, instrument_map, actual_as_of,
            ...
            self.store.record_failure(
                exc.reasons, as_of_date=exc.as_of, coverage=exc.coverage,
            )
            raise
        filters = {
            "universe": "all_cn_stocks", "include_bj": cfg.after_close_include_bj,
            "exclude_st": True,
        ...
        csi800: dict[str, Any] = {
            "status": "unavailable", "reason": "中证800点时成分尚未缓存",
        }
        try:
        ...
        snapshot_id = "ac_" + hashlib.sha256(
            f"{actual_as_of}:{SCORE_VERSION}:{input_hash}".encode()
        ).hexdigest()[:24]
        ...
        concentration = (
            max(Counter(primary_l1).values()) / len(primary_l1) if primary_l1 else None
        )
        l1_returns = [
        ...
        snapshot = AfterCloseSnapshot(
            snapshot_id=snapshot_id, as_of_date=actual_as_of, input_hash=input_hash,
    ...
    def _write_research_lake(
        self, snapshot: AfterCloseSnapshot, frame: pd.DataFrame, boards: list[dict[str, Any]],
    ) -> None:
        try:
            ...
            rows = [
                ...
                for board in boards for symbol in self._board_members(board)
            ]
            if rows:
                lake.write_partition(
                    ArtifactKind.RAW, AssetClass.STOCK, Frequency.DAILY,
    ...
    def evaluate_pending(self, frame: pd.DataFrame) -> None:
        ...
        for meta in self.store.history(100):
            ...
            if snapshot is None:
                continue
            existing = {
                item["horizon"]: item for item in self.store.labels(snapshot.snapshot_id)
            }
            ...
            if snapshot.validation.get("baselines", {}).get("csi800", {}).get("status") == "available":
                try:
                    ...
                    csi_symbols = set(load_cached_csi800_members_as_of(
                        snapshot.as_of_date, pull=False,
                    )["symbols"])
                except (ImportError, OSError, RuntimeError, TypeError, ValueError):
                    csi_symbols = set()
            for horizon in (1, 3, 5, 7):
                if horizon in existing and (
                    not csi_symbols or existing[horizon].get("csi800_mean_return") is not None
                ):
                    continue
                returns = []
                drawdowns = []
                for candidate in snapshot.candidates:
                    ...
                    if values is None:
                        continue
                    future = values.loc[
                        values.index > pd.Timestamp(snapshot.as_of_date), "close"
                    ].head(horizon)
                    base = values.loc[
                        values.index <= pd.Timestamp(snapshot.as_of_date), "close"
                    ].tail(1)
                    if len(future) < horizon or base.empty:
                        continue
                    path = pd.concat((base, future)).astype(float)
                    returns.append(float(future.iloc[-1] / base.iloc[-1] - 1))
                    drawdowns.append(float((path / path.cummax() - 1).min()))
                if len(returns) != len(snapshot.candidates) or not returns:
                    continue
                market_returns = []
                ...
                self.store.save_labels(snapshot.snapshot_id, horizon, {
                    "candidate_count": len(returns),
                    "mean_return": _finite(np.mean(returns)),
                    ...
                    "excess_mean_return": _finite(
                        np.mean(returns) - market_mean if market_mean is not None else np.nan
                    ),
                    "mean_max_drawdown": _finite(np.mean(drawdowns)),
                    "capacity_avg_amount_20d": _finite(np.mean([
                        item.metrics.get("avg_amount_20d")
                        for item in snapshot.candidates
                        if item.metrics.get("avg_amount_20d") is not None
                    ])),
                    "candidate_turnover": snapshot.validation.get("candidate_turnover"),
                    "sector_concentration": snapshot.validation.get("sector_concentration"),
                    "baseline": "all_market",
                    "csi800_status": "available" if csi_mean is not None else "unavailable",
                    "csi800_member_returns": len(csi_returns),
                    "csi800_mean_return": csi_mean,
                    "excess_vs_csi800": _finite(
                        np.mean(returns) - csi_mean if csi_mean is not None else np.nan
                    ),
                })
