        ...
        "sections": (
            {
                ...
                "items": (
                    ...
                    (
                        "首次同步的原生更新器运行上限由 30 分钟延长到 60 分钟，"
                        "仍保留有界终止与失败恢复。"
                    ),
                ),
            },
        ...
        "sections": (
            {
                ...
                "items": (
                    ...
                    (
                        "更新 helper 直接从已验证的目标发行槽自举，并沿用当前 runtime-worker "
                        "的数据根和 IPC 配置完成排空、恢复与新一代启动。"
                    ),
                    (
                        "激活使用完整窗口等待十秒排空预算；边界回包、既有应用更新冻结和"
                        "维护进入回复短暂丢失均可有界恢复，不再误判失败。"
                    ),
                    (
                        "维护预检、身份与数据根检查继续 fail closed；排空无法确认时会报告"
                        "协调阶段并安全回滚，非更新来源的维护冻结仍明确阻断。"
                    ),
                ),
            },
            {
                "title": "本地 StockDB 与正式研究",
                "items": (
                    (
                        "StockDB v2 验收记录在标的覆盖率不低于 98%、必要 OHLCV 完整且"
                        "交易日一致时即可作为正式研究证据；100% 覆盖继续作为完整度遥测。"
                    ),
                    (
                        "今天页面刷新跟随本地已验收交易日；正式研究直接使用 StockDB 原生"
                        "前复权行情并规范化严格停牌占位行，不要求在线数据源补充。"
                    ),
                ),
            },
            {
                ...
                "items": (
                    (
                        "恢复内置新闻源的真实抓取与有界瞬态传输重试，官方 JSON、"
                        "二进制响应和深交所首个限量窗口按既有契约解析。"
                    ),
                    (
