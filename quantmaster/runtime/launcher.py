...
def _launcher_script() -> str:
    ...
    return (
        ...
        "\"%QM_APP_DIR%\" >nul 2>&1\r\n"
        "if not errorlevel 1 (\r\n"
        "    call :qm_fail 6 \"application root is a reparse point\"\r\n"
        "    exit /b 6\r\n"
        ")\r\n"
        "set \"QM_TARGET=\"\r\n"
        "set \"QM_LINES=\"\r\n"
        ...
        "do set \"QM_LINES=%%N\"\r\n"
        "if not \"%QM_LINES%\"==\"1\" (\r\n"
        "    call :qm_fail 3 \"launcher.target must contain exactly one lowercase SHA line\"\r\n"
        "    exit /b 3\r\n"
        ")\r\n"
        "for /f \"usebackq delims=\" %%A in (\"%QM_APP_ROOT%launcher.target\") "
        "do set \"QM_TARGET=%%A\"\r\n"
        "set \"QM_INVALID=\"\r\n"
        "for /f \"usebackq delims=0123456789abcdef\" %%A in "
        "(\"%QM_APP_ROOT%launcher.target\") do set \"QM_INVALID=%%A\"\r\n"
        "if defined QM_INVALID (\r\n"
        "    call :qm_fail 3 \"launcher.target contains a non-hex character\"\r\n"
        "    exit /b 3\r\n"
        ")\r\n"
        "if not defined QM_TARGET (\r\n"
        "    call :qm_fail 3 \"launcher.target is not a lowercase 40-character SHA\"\r\n"
        "    exit /b 3\r\n"
        ")\r\n"
        "set \"QM_SLOTS=%QM_APP_ROOT%slots\"\r\n"
        "if not exist \"%QM_SLOTS%\\\" (\r\n"
        "    call :qm_fail 4 \"the immutable slots directory is missing\"\r\n"
        "    exit /b 4\r\n"
        ")\r\n"
        "\"%SystemRoot%\\System32\\fsutil.exe\" reparsepoint query \"%QM_SLOTS%\" >nul 2>&1\r\n"
        "if not errorlevel 1 (\r\n"
        "    call :qm_fail 6 \"the immutable slots directory is a reparse point\"\r\n"
        "    exit /b 6\r\n"
        ")\r\n"
        "set \"QM_SLOT=%QM_SLOTS%\\%QM_TARGET%\"\r\n"
        "if not exist \"%QM_SLOT%\\\" (\r\n"
        "    call :qm_fail 4 \"the target immutable slot is missing\"\r\n"
        "    exit /b 4\r\n"
        ")\r\n"
        "\"%SystemRoot%\\System32\\fsutil.exe\" reparsepoint query \"%QM_SLOT%\" >nul 2>&1\r\n"
        "if not errorlevel 1 (\r\n"
        "    call :qm_fail 6 \"the target immutable slot is a reparse point\"\r\n"
        "    exit /b 6\r\n"
        ")\r\n"
        "set \"QM_EXE=%QM_SLOT%\\QuantMaster.exe\"\r\n"
        "if not exist \"%QM_EXE%\" (\r\n"
        "    call :qm_fail 4 \"the target immutable slot has no QuantMaster.exe\"\r\n"
        "    exit /b 4\r\n"
        ")\r\n"
        "\"%SystemRoot%\\System32\\fsutil.exe\" reparsepoint query \"%QM_EXE%\" >nul 2>&1\r\n"
        "if not errorlevel 1 (\r\n"
        "    call :qm_fail 6 \"the target executable is a reparse point\"\r\n"
        "    exit /b 6\r\n"
        ")\r\n"
        "pushd \"%QM_SLOT%\" >nul\r\n"
        "if errorlevel 1 (\r\n"
        "    call :qm_fail 5 \"cannot enter the target immutable slot\"\r\n"
        "    exit /b 5\r\n"
        ")\r\n"
        "\"%QM_EXE%\" serve --open %*\r\n"
        "set \"QM_EXIT_CODE=%ERRORLEVEL%\"\r\n"
        "if not \"%QM_EXIT_CODE%\"==\"0\" echo QuantMaster launcher: application exited with "
        "code %QM_EXIT_CODE%.\r\n"
        "popd >nul\r\n"
        "exit /b %QM_EXIT_CODE%\r\n"
        ":qm_fail\r\n"
        "echo QuantMaster launcher error [%~1]: %~2\r\n"
        "if /i not \"%QM_LAUNCHER_NO_PAUSE%\"==\"1\" pause\r\n"
        "exit /b %~1\r\n"
    )
