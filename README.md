...
| 📖 内置帮助 | 页头“帮助”提供 16 章量化研究与 A 股交易系统手册、稳定小节深链、全文搜索、可复制代码、32 道自测，以及 6 个计算器和 OLS、BH-FDR、概率校准、双资产风险 4 个本地实验台 |
...
桌面端推荐启动 `qm serve` 后从顶部「候选」管理研究范围，从「设置」校验全部字段、读取模型列表、
...
手动全量刷新；单标的响应校验成功前不会替换旧缓存，任务可取消、续跑和重试失败项。
...
2. 飞书管理员私聊先生成绑定码并发送 `绑定 QuantMaster XXXXXXXX`，再由管理员到目标群完成群绑定。
   资讯卡片直接展示摘要、利好 / 利空 / 中性判断、情绪值、重要度、相关标的、核查依据和原文链接。
...
顶部「资讯」集中展示事件流、标注状态、大盘情绪、申万一级板块分数、相关标的和来源健康度。
...
暂停和手动运行统一在「自动化」页面管理。资讯扫描只在发现实际内容时推送摘要，不再额外
发送泛化的任务完成通知。结构化资讯长期保留；原始响应默认缓存 7 天并
在每日清理任务中回收。大盘与板块分数使用情绪、置信度、重要度、来源权重和 3 日半衰期
统一计算，范围为 -100 到 +100。`news_sentiment` 因子只读取本地资讯库，按首次获取时间对齐交易日，
