...
def test_cn_minute_aggregation_never_crosses_lunch():
    index = pd.to_datetime([
        "2026-08-07 11:26", "2026-08-07 11:29",
        "2026-08-07 13:00", "2026-08-07 13:04",
    ])
    frame = pd.DataFrame({
        "open": [1, 2, 3, 4], "high": [2, 3, 4, 5], "low": [1, 1, 3, 3],
        "close": [2, 3, 4, 5], "volume": [1, 1, 1, 1], "amount": [2, 3, 4, 5],
    }, index=index)
    result = FreeStockDBSource._aggregate_cn_minutes(frame, "5m", {
...
def test_ingest_store_is_immutable_content_addressed_and_reusable(tmp_path, isolated_config):
    ...
    kwargs = dict(
        boards=[{"code": "L1", "members": ["600000.SH"]}], catalog=[], delisted=[],
        as_of_date="2026-08-07", artifact_id="artifact", master_snapshot_id="master",
        start_date="2026-01-01", end_date="2026-08-08", coverage={"status": "complete"},
    ...
    revised = store.publish(frame=frame.assign(close=10.6), **{**kwargs, "provenance": {**kwargs["provenance"], "cache_key": "revised"}})
...
def test_ingest_prune_uses_publish_time_not_content_id(tmp_path, isolated_config):
    ...
    for index in range(3):
        snapshot = store.publish(
            ...
            boards=[{"code": "L1"}], catalog=[], delisted=[], as_of_date="2026-08-07",
            artifact_id=f"artifact-{index}", master_snapshot_id="master",
            start_date="2026-01-01", end_date="2026-08-08", coverage={},
    ...
    assert manifests[1].exists() and manifests[2].exists()
...
def test_frozen_factor_derives_research_price_without_mutating_raw_frame():
    raw = pd.DataFrame({
        ...
        "open": [10.0, 5.0], "high": [10.0, 5.0], "low": [10.0, 5.0],
        "close": [10.0, 5.0], "pre_close": [10.0, 10.0],
    })
...
def test_research_adapter_reuses_ingest_and_exposes_factor_lineage(tmp_path, isolated_config):
    ...
    raw = pd.DataFrame({
        ...
        "volume": [100.0], "amount": [1_000.0], "turnover": [1.0],
    })
    factors = pd.DataFrame({
        "symbol": ["600000.SH"], "date": [pd.Timestamp("2026-07-01")],
        "adj_factor": [2.0],
    })
    snapshot = ingest_store.publish(
        frame=raw, adjustment=factors, boards=[{"code": "L1"}], catalog=[], delisted=[],
        as_of_date="2026-08-07", artifact_id="artifact", master_snapshot_id="master",
        start_date="2026-01-01", end_date="2026-08-08", coverage={}, provenance={},
    )
    adapter = StockDBResearchAdapter(
        ResearchCatalog(tmp_path / "research.sqlite"), source=_NoReadSource(),
        instruments=SimpleNamespace(list=lambda **_kwargs: []), ingest_store=ingest_store,
    )
...
@pytest.mark.parametrize(
    ("name", "category"),
    [
        ("沪深300ETF", "境内宽基"), ("恒生科技ETF(QDII)", "港股及海外 QDII"),
        ("国债ETF", "债券"), ("黄金ETF", "商品"), ("货币ETF", "货币"),
        ("红利低波ETF", "策略"), ("机器人ETF", "行业主题"),
    ],
)
...
def test_lof_is_not_admitted_as_etf():
    lof = Instrument(
        "501001.SH", "501001", "某某LOF", "CN", "SH", "fund", status="listed",
    )
...
class _EtfSource:
        ...
        for index, symbol in enumerate(("510300.SH", "159920.SZ", "511010.SH"), 1):
            for offset, stamp in enumerate(dates):
                ...
                rows.append({
                    ...
                    "pe_ttm": np.nan, "pb": np.nan, "is_st": False,
                    "pre_close": close - .001, "pct_chg": .1, "amplitude": 1,
                    "turnover": .1, "vol_ratio": 1, "name": symbol,
                })
    ...
    def intraday_many(self, symbols, start, end, frequency):
        ...
        return pd.DataFrame({
            "symbol": "510300.SH", "date": stamps, "open": 1.0, "high": 1.01,
            "low": .99, "close": np.linspace(1, 1.01, len(stamps)),
...
def test_etf_scan_includes_qdii_uses_category_rank_and_keeps_minute_non_scoring(
    ...
    monkeypatch.setattr(EtfResearchService, "_direct_share_observations", staticmethod(lambda: pd.DataFrame()))
    service = EtfResearchService(
        source=_EtfSource(), instruments=_EtfInstruments(),
        ingest_store=StockDBIngestStore(tmp_path / "ingest"),
    ...
    monkeypatch.setattr(
        "quantmaster.rotation.etf_research.get_etf_research_service", lambda: service,
    )
...
def test_experimental_tick_is_quotad_cached_and_audited(tmp_path, isolated_config):
    ...
    audits = [json.loads(line) for line in (tmp_path / "experimental" / "audit.jsonl").read_text(
        encoding="utf-8"
    ).splitlines()]
...
def test_stockdb_audit_api_discloses_same_upstream_and_experimental_state(
    ...
    monkeypatch.setattr(FreeStockDBSource, "board_hierarchy", lambda self: [
    ...
    monkeypatch.setattr(
        FreeStockDBSource, "artifact_identity",
        lambda self, **kwargs: StockDBArtifactIdentity.discover(
            None, None, data_session=kwargs.get("data_session", ""),
        ),
    )
    ...
    assert payload["capabilities"]["realtime_tick"] == "disabled"
